 ...
 */
export function useAccountTypesList(search?: string, ordering?: string, options?: { enabled?: boolean; suspense?: boolean; refetchInterval?: number; initialData?: z.infer<typeof AccountTypesListResponseSchema> }) {
  const { initialData, ...restOptions } = options ?? {}

  return useQuery({
    queryKey: ['accountTypesList', search, ordering],
    queryFn: async ({ signal }) => {
      try {
        const result = await accountTypesList({ params: { query: { search, ordering } } })
        return result
      } catch (error) {
 ...
 */
export function useSuspenseAccountTypesList(search?: string, ordering?: string, options?: { enabled?: boolean; suspense?: boolean; refetchInterval?: number; initialData?: z.infer<typeof AccountTypesListResponseSchema> }) {
  const { initialData, ...restOptions } = options ?? {}
