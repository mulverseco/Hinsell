 ...
 */
export function useBudgetsList(search?: string, ordering?: string, options?: { enabled?: boolean; suspense?: boolean; refetchInterval?: number; initialData?: z.infer<typeof BudgetsListResponseSchema> }) {
  const { initialData, ...restOptions } = options ?? {}

  return useQuery({
    queryKey: ['budgetsList', search, ordering],
    queryFn: async ({ signal }) => {
      try {
        const result = await budgetsList({ params: { query: { search, ordering } } })
        return result
      } catch (error) {
 ...
 */
export function useSuspenseBudgetsList(search?: string, ordering?: string, options?: { enabled?: boolean; suspense?: boolean; refetchInterval?: number; initialData?: z.infer<typeof BudgetsListResponseSchema> }) {
  const { initialData, ...restOptions } = options ?? {}
