 ...
 */
export function useOpeningBalancesList(search?: string, ordering?: string, options?: { enabled?: boolean; suspense?: boolean; refetchInterval?: number; initialData?: z.infer<typeof OpeningBalancesListResponseSchema> }) {
  const { initialData, ...restOptions } = options ?? {}

  return useQuery({
    queryKey: ['openingBalancesList', search, ordering],
    queryFn: async ({ signal }) => {
      try {
        const result = await openingBalancesList({ params: { query: { search, ordering } } })
        return result
      } catch (error) {
 ...
 */
export function useSuspenseOpeningBalancesList(search?: string, ordering?: string, options?: { enabled?: boolean; suspense?: boolean; refetchInterval?: number; initialData?: z.infer<typeof OpeningBalancesListResponseSchema> }) {
  const { initialData, ...restOptions } = options ?? {}
