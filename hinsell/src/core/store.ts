...
// This file sets up a comprehensive global state management using Zustand for an e-commerce app.
// We've sliced the state into logical modules (cart, currency, items, inventory) for modularity,
// while keeping them in a single store file for simplicity. Each slice has its own state and actions.
// Persistence is added via zustand/middleware for cart and user preferences.
// Types are inferred from Zod schemas for type safety.
// Async actions (e.g., for API integrations) are handled within actions using promises.
...
const convert = (amount: string, rateFrom: string, rateTo: string): string => {
  ...
  const numFrom = parseFloat(rateFrom);
  const numTo = parseFloat(rateTo);
    ...
    persist(
      (set, get) => ({
        ...
        updateBalance: (variantId, updates) =>
          set((state) => ({
            balances: state.balances.map((b) =>
              b.variant === variantId ? { ...b, ...updates } : b
            ),
          })),
        checkAvailability: (variantId, quantity) => {
          ...
          return parseFloat(balance.available_quantity) >= quantity;
        },
          ...
          set((state) => ({
            balances: state.balances.map((b) =>
              b.variant === variantId
                ? {
                    ...b,
                    available_quantity: (parseFloat(b.available_quantity || '0') - quantity).toString(),
                    reserved_quantity: (parseFloat(b.reserved_quantity || '0') + quantity).toString(),
                  }
                : b
            ),
          }));
        ...
        updateQuantity: (variantId, newQuantity) => {
          ...
          const totalPrice = (parseFloat(price) * newQuantity).toString();
          set((state) => ({
        ...
        calculateTotal: () => {
          ...
          return cartItems
            ...
            .toString(); // Add conversion if needed: convertAmount(total, 'base', currentCurrency?.code || '');
        },
        clearCart: () => {
