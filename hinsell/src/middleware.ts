...
import { auth as middleware  } from "@/core/auth"
...
export async function middleware(request: NextRequest) {
  ...
  // Skip middleware for Auth.js routes but allow locale handling
  ...
  let locale = cookieLocale && availableLocales.includes(cookieLocale) ? cookieLocale : defaultLocale
  ...
  const pathnameHasLocale = availableLocales.some((loc) => pathname.startsWith(`/${loc}/`) || pathname === `/${loc}`)
...
function isCLP(request: NextRequest): boolean {
  ...
  const isInternalRoute =
    cleanPath.startsWith("/category/clp/") || cleanPath.includes("/clp") || cleanPath.includes("/plp")
...
function isPLP(request: NextRequest): boolean {
  ...
  const isCategory = cleanPath.startsWith("/category/")
  const isInternalRoute =
    cleanPath.startsWith("/category/plp/") || cleanPath.includes("/clp") || cleanPath.includes("/plp")
