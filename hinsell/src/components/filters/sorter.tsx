    ...
    <div className={className}>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <div className="flex cursor-pointer flex-nowrap items-center justify-center gap-1 px-2 py-1.5 font-medium [&[data-state=open]>svg]:rotate-180">
            <span>{sortBy ? LABELS[sortBy as keyof typeof LABELS] : "Sort by"}</span>
            <ChevronDown />
          </div>
        </DropdownMenuTrigger>
        <DropdownMenuContent
          className="w-[240px] rounded-md border-none p-0 text-sm font-medium tracking-tight text-primary shadow-2xl ring-1 ring-black/10"
          align="end"
        >
          {Object.entries(LABELS).map(([key, label]) => (
            <DropdownMenuItem
              key={label}
              className="cursor-pointer px-3.5 py-3 text-sm last:border-b-0 "
              onClick={() => {
